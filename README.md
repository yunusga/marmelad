...
Сборка стилей осуществляется при помощи [Gulp.js](https://gulpjs.com/).
Для стилей используется препроцессор [Stylus](http://stylus-lang.com/).<br>
Так же поддерживаются нижеследующие препроцессоры:
1) [gulp-stylus](https://www.npmjs.com/package/gulp-stylus)
2) [gulp-postcss](https://github.com/postcss/gulp-postcss)
3) [gulp-sass](https://www.npmjs.com/package/gulp-sass)
и.т.д

**Marmelad** компилирует все файлы стилей с расширениями **(.scss,.styl)** в **.css** файл.
К примеру мы создали стили для шапки в папке мармелада **(projectFolder/marmelad/boilerplate/_blocks/_app-header/_app-header.styl)**, 
...
Сборка плагинов осуществляется примерно таким же образом, только за одним исключением.
