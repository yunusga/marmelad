...
Заготовка фронтенд проекта для продвинутых и начинающих 🤘. Хорошо подходит для поддержания единой структуры проектов в команде и легкого переиспользования готовых блоков между проектами блогадаря БЭМ подходу к организации файловой структуры и данных проекта.

Звисисмости marmelad устанавливаются один раз и в будущем только обновляются, т.е. нет необходимости устанавливать кучу npm зависимостей для каждого проекта. Как следствие, экономия дискового пространства и возможность создания и работы в проектах без интернета.

## Содержание

- [Установка](#установка)
  - [Проблемы установки глобальнах модулей для macOS и Linux](#проблемы-установки-глобальнах-модулей-для-macos-и-linux)
...
### Проблемы установки глобальнах модулей для macOS и Linux
...
Ознакомтесь с руководством NPM для установки глобальных модулей [Resolving EACCES permissions errors when installing packages globally](https://docs.npmjs.com/resolving-eacces-permissions-errors-when-installing-packages-globally)
...
> Эксперементальная команда, пока никаких опций передать нельзя.

- Форматирование HTML-кода страниц проекта в папке сборки (`static`)
- Прописывание хешей для подключаемых файлов, для сброса кеша
